...
!-----------------------------------------------------------------------
      ...
      Real(Double) fpar(16), r(n), delx(n), sx, distdot
      external distdot
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      implicit none
      integer i,n,ipar(16)
      Real(Double) fpar(16),sol(n),delx(n)
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      implicit none
      integer ipar(16)
      Real(Double) alpha, beta, zero, one
...
!
      ...
      Real(Double) umm,beta,ypiv(*),u(*),x, xpiv
...
!-----------------------------------------------------------------------
      ...
      Integer :: Lbp,n
      Real(Double) p(n,lbp), y(*), u(*), usav(*), x, flops
      integer k,np,npm1,j,ju,indp
!-----------------------------------------------------------------------
...
!-------end-of-uppdir---------------------------------------------------
      ...
      Real(Double)  ops,hh(m),vec(lda,m)
!-----------------------------------------------------------------------
...
!
!     External routines used: Real(Double) distdot
!-----------------------------------------------------------------------
      integer i,k
      Real(Double)  nrm0, nrm1, fct, thr, distdot, zero, one, reorth
...
!      integer n, ipar(16)
!      Real(Double) rhs(n), sol(n), fpar(16), w(*)
!     Where
...
!
!     Real(Double) function distdot(n,x,ix,y,iy)
!     integer n, ix, iy
!     Real(Double) x(1+(n-1)*ix), y(1+(n-1)*iy)
!
!     This interface of DISTDOT is exactly the same as that of
!     DDOT (or SDOT if real == Real(Double)) from BLAS-1. It should have
...
!-----------------------------------------------------------------------
      ...
      Implicit None
      Integer n, ipar(16)
      Real(Double) rhs(n), sol(n), fpar(16), w(n,8)
!-----------------------------------------------------------------------
...
!
      Integer i
      Real(Double) alpha,beta,rho,omega
      Logical lp, rp
      Save lp, rp
!
!     where to go
...
!
      Integer i
      Logical lp, rp
      Real(Double) eta,sigma,theta,te,alpha,rho,tao
      Save
!
...
!-----------------------------------------------------------------------
      ...
      Implicit None
      Integer n,ipar(16)
      Real(Double) rhs(n), sol(n), fpar(16), w(n,*)
!-----------------------------------------------------------------------
! Quasi GMRES method for solving a linear
! system of equations a * sol = y.  Real(Double) version.
...
!
      ...
      parameter(one=1.0D0,zero=0.0D0)
!
      Real(Double) t,distdot,ss,res,beta,ss1,delta,x,zeta,umm
      integer k,j,i,i2,ip2,ju,lb,lbm1,np,indp
      logical lp,rp,full, perm(3)
      Real(Double) ypiv(3),u(3),usav(3)
      save
!
!     where to go
!
...
!-----------------------------------------------------------------------
      ...
      implicit none
      integer n, ipar(16)
      real(Double) rhs(n),sol(n),fpar(16),wk(n,*)
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      implicit none
      integer n, ipar(16)
      real(Double)  fpar(16), rhs(n), sol(n), w(n,*)
!-----------------------------------------------------------------------
